      ...
      try {
        ...
        if (isBulletLine) {
          ...
          if (!contentAfterBullet) {
            ...
            // 내용이 있는 불릿이면 Enter 후에 새 불릿 추가
            const indent = bulletMatch[1].match(/^\s*/)[0];
              ...
              try {
                ...
                if (newElement) {
                  ...
                  if (newElement.tagName?.toLowerCase() !== 'p') {
                    ...
                    newP.style.margin = '0';
                    newP.style.lineHeight = '1.6';
                    newP.textContent = bulletText;
                    
                    newElement.replaceWith(newP);
                    
                    // 커서를 불릿 뒤로 이동
                    const finalRange = document.createRange();
                    finalRange.setStart(newP.firstChild, bulletText.length);
                    finalRange.collapse(true);
                    
                    newSelection.removeAllRanges();
                    newSelection.addRange(finalRange);
                  } else {
                    newElement.textContent = bulletText;
                    
                    // 커서를 불릿 뒤로 이동
                    const finalRange = document.createRange();
                    finalRange.setStart(newElement.firstChild, bulletText.length);
                    finalRange.collapse(true);
